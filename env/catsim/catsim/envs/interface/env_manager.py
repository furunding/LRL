...
class EnvManager(object):
    def __init__(self, 
        ...
        base_map_port, 
        scene_name, 
        docker_hostname='trsuser5',
        image_name='combatmodserver:1.3.0.1',
        docker_name_prefix = 'docker_env_', 
        sim_server_ip='127.0.0.1'
    ):
        """基本设置"""
        # base_map_port为基准端口号, 由此生成映射到容器内部各端口的宿主机端口号
        # scene_name为想定文件的绝对路径;
        # prefix为容器管理脚本manage_client(对战框架的一部分)所在路径
        # image_name为docker镜像名称
        self.env_id = env_id
        self.base_map_port = base_map_port
        self.scene_name = scene_name
        self.docker_hostname = docker_hostname
        self.image_name = image_name
        self.sim_server_ip = sim_server_ip

        self.docker_name = docker_name_prefix + str(env_id)
        self.ports = [(self.base_map_port + self.env_id * 10 + i) for i in range(4)]
    ...
    def open(self):
        ...
        #     self.prefix, self.sim_server_ip, self.ports[0], self.scene_name)
        # # EnvManager._exec_command(open_cmd)
        # while os.system(open_cmd) != 0:
        #     time.sleep(1)

        sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        sock.connect((self.sim_server_ip, self.ports[0]))
    ...
    def start(self):
        # start_cmd = '{}/manage_client -host {} -port {} start'.format(self.prefix, self.sim_server_ip, self.ports[0])
    ...
    def pause(self):
        # cmd = '{}/manage_client -host {} -port {} pause'.format(self.prefix, self.sim_server_ip, self.ports[0])
    ...
    def resume(self):
        # cmd = '{}/manage_client -host {} -port {} resume'.format(self.prefix, self.sim_server_ip, self.ports[0])
        # EnvManager._exec_command(cmd)
    ...
    def stop(self):
        # cmd = '{}/manage_client -host {} -port {} stop'.format(self.prefix, self.sim_server_ip, self.ports[0])
        # EnvManager._exec_command(cmd)
    ...
    def close(self):
        # cmd = '{}/manage_client -host {} -port {} close'.format(self.prefix, self.sim_server_ip, self.ports[0])
        # EnvManager._exec_command(cmd)
    ...
    def reset(self):
        """容器化仿真环境重置"""
        if 'sim_fast' in self.image_name:  # 极速版重启方法
