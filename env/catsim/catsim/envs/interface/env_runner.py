...
class EnvRunner(object):
        ...
        for agent in self.agents:
            ...
            if side == 'red':
                ...
                try:
                    ...
                    self.logger.debug("在{}秒时蓝方智能体决策超时\n".format(sim_time))  # 记录超时信息
                # 指令检查
    ...
    def _get_done(observation):
        ...
        red_score = (len([u for u in obs_red['units'] if u['LX'] != 41 and u['LX'] != 14])+obs_red['airports'][0]['NM'])/41*10 + (2-len(cmd_posts))*30
    ...
    def _validate_deploy(side, pos2d, sim_time):
        if sim_time > 120 or (side == 'red' and pos2d[0] < 0) or (side == 'blue' and pos2d[0] > 0):
            print("护卫舰初始部署指令无效")
        ...
        if maintype == 'airattack':
            ...
            if len(unit) == 0 and len(team) == 0:
                ...
                if obj['LX'] not in type4cmd[maintype]:
                    print("类型为%s的平台或者编队%s无法执行%s指令" % (obj['LX'], self_id, maintype))
                    return True
                # 检查速度设置是否越界
